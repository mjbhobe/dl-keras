...
RESHAPED = IMAGE_WIDTH * IMAGE_HEIGHT * NUM_CHANNELS
NUM_CLASSES = 10
...
X_train = X_train.reshape(X_train.shape[0], RESHAPED).astype('float32') / 255.0
X_test = X_test.reshape(X_test.shape[0], RESHAPED).astype('float32') / 255.0
...
if DO_TRAINING:
      def build_model():
            model = Sequential([
                  Dense(512, input_shape=(RESHAPED,), activation='relu'),
